...
def _toy_df():
    ...
    return pd.DataFrame({
        ...
        "marital-status": ["Never-married", "Married-civ-spouse", "Married-civ-spouse", "Never-married"],
        "occupation": ["Prof-specialty", "Adm-clerical", "Exec-managerial", "Handlers-cleaners"],
...
def test_inference_output_shape_and_binary():
    ...
    X, y, enc, lb = process_data(df, CAT_FEATURES, label="salary", training=True)
    model = train_model(X, y)
    X2, y2, _, _ = process_data(df, CAT_FEATURES, label="salary", training=False, encoder=enc, lb=lb)
